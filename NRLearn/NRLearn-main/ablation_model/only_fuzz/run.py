    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            ...
            if train_with_mask:
                ...
                print("mask_shape")
                print(vul_query_mask.shape)
                print("input_shape")
                print(input_ids.shape)
                print("output_shape")
                print(repair_input_ids.shape)                                   
...
def main():
    ...
    model = Transformer(src_vocab_size=tokenizer.vocab_size+10,
                        trg_vocab_size=tokenizer.vocab_size+10,
                        src_pad_idx=tokenizer.pad_token_id,
                        trg_pad_idx=tokenizer.pad_token_id,
                        )

    if args.load_pretrained_t5:
        model.load_state_dict(torch.load(f"saved_models/checkpoint-best-loss/{args.pretrained_model_name}", map_location=args.device))
    
    model = NRLearn(trans=model, tokenizer=tokenizer, args=args)
