...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):     
        return torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].vul_query_label), torch.tensor(self.examples[i].repair_input_ids), torch.tensor(self.examples[i].commit_index)
        ...
        for step, batch in enumerate(bar):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.save_steps == 0 :
                    ...
                    if idx >=3:
                        confidence_list2 = prevaluate(args, model, loc_model, eval_dataset, tokenizer, idx, eval_when_training=True, eval_with_mask=train_with_mask) 
                    eval_loss =  evaluate(args, model, loc_model, eval_dataset, tokenizer, confidence_list2,idx, eval_when_training=True, eval_with_mask=train_with_mask)
...
def evaluate(args, model, loc_model, eval_dataset, tokenizer,confidence_list,idx, eval_when_training=False, eval_with_mask=False):
    #build dataloader
    eval_sampler = SequentialSampler(eval_dataset)
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, num_workers=0)
    # multi-gpu evaluate
