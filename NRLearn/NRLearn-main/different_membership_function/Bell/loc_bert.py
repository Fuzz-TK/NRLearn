...
class ConsequentLayer(torch.nn.Module):
    ...
    def __init__(self, d_in, d_rule, d_out):
        ...
        #print(c_shape)
        c_shape = tuple(int(dim) for dim in c_shape)
    ...
    def forward(self, x):
        ...
        # Append 1 to each list of input vals, for the constant term:
        device = "cuda:0"
        ones_tensor = torch.ones(x.shape[0], 1).to(device)
        x_plus = torch.cat([x, ones_tensor], dim=1)
...
class LocModel(nn.Module):
    def __init__(self, encoder, config, tokenizer, args, num_labels,description,outvarnames,hybrid,invardefs):
        ...
        #print(invardefs)
        self.num_rules = np.prod([len(mfs) for _, mfs in invardefs], dtype=np.int64)
        if self.hybrid:
    ...
    def forward(self, input_ids, vul_query_label=None):
        ...
        #print(vul_query_prob)
        self.fuzzified = self.layer['fuzzify'](outputs)
        self.raw_weights = self.layer['rules'](self.fuzzified)
        self.weights = F.normalize(self.raw_weights, p=1, dim=1)
        self.rule_tsk = self.layer['consequent'](outputs)
        y_pred = torch.bmm(self.rule_tsk, self.weights.unsqueeze(2))
        print(self.fuzzified.shape)
        print(self.raw_weights.shape)
        vul_query_prob = y_pred.squeeze(2)
        if vul_query_label is not None:
            loc_loss_fct = nn.CrossEntropyLoss()
            print(f"vul_query_prob size: {vul_query_prob.shape}")
            print(f"vul_query_label size: {vul_query_label.shape}")
            #print(vul_query_prob)
            #print(vul_query_label)
            #assert vul_query_label.dim() == 1 and vul_query_label.dtype == torch.long, "vul_query_label
            criterion = nn.BCEWithLogitsLoss()
            vq_loss = criterion(vul_query_prob, vul_query_label)
            return vq_loss
        else:
            #vul_query_mask = self.mask_activation(vul_query_prob)
            self.fuzzified = self.layer['fuzzify'](vul_query_prob)
            self.raw_weights = self.layer['rules'](self.fuzzified)
            self.weights = F.normalize(self.raw_weights, p=1, dim=1)
            self.rule_tsk = self.layer['consequent'](vul_query_prob)
            y_pred = torch.bmm(self.rule_tsk, self.weights.unsqueeze(2))
            vul_query_mask = y_pred.squeeze(2)
