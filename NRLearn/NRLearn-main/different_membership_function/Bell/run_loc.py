...
class BellMembFunc(nn.Module):
    '''
        Bell-shaped membership functions, defined by three parameters:
            a, the width
            b, the shape
            c, the center
    '''
    ...
    def forward(self, x):
        val = 1 / (1 + torch.abs((x - self.c) / self.a)**(2 * self.b))
        return val
...
def make_bell_mfs(a, b, c_list):
    '''Return a list of bell-shaped mfs, same a and b, list of c'''
    return [BellMembFunc(a, b, c) for c in c_list]
...
def main():
    ...
    '''
    a, b = 1, 2  # Example parameters for the bell-shaped functions
    c_list = torch.linspace(0, 512, 1).tolist()  # Reduced number of membership functions
    bell_mfs = make_bell_mfs(a, b, c_list)

    # Create invars with fewer bell-shaped membership functions
    invars = []
    for i in range(512):
        invars.append(('x{}'.format(i), bell_mfs))
    ...
    if args.load_pretrained_model:
        ...
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
        model.load_state_dict(torch.load(output_dir, map_location=args.device))  # 这是实际加载模型权重的部分。
