...
class TriangularMembFunc(torch.nn.Module):
    '''
        Triangular membership functions, defined by three parameters:
            a, the left endpoint
            b, the peak (center)
    ...
    def forward(self, x):
        ...
        val = torch.max(torch.min(left, right), torch.tensor(0.0))
        return val

    def pretty(self):
        return 'TriangularMembFunc {} {} {}'.format(self.a, self.b, self.c)

def make_triangular_mfs(a, b, c_list):
    '''Return a list of triangular mfs, same a and b, list of c'''
    return [TriangularMembFunc(a, b, c) for c in c_list]
...
def main():
    ...
    '''
    a, b = 0, 512  
    c_list = torch.linspace(0, 512, 1).tolist()
    for i in range(512):
        invars.append(('x{}'.format(i), make_triangular_mfs(a, b, c_list)))
